...
- 데이터 증강(Augmentation): 벡터화 전, 각 규정의 `page_content`에 사용자가 자주 묻는 구체적인 하위 품목 예시(동의어)를 추가 (예: "액체·분무·겔류" ➔ "액체·분무·겔류 (고추장, 김치, 샴푸, 화장품, 향수 등)").
- 임베딩: OpenAI의 text-embedding-3-small 또는 유사한 성능의 임베딩 모델을 사용하여 증강된 page_content 텍스트를 벡터로 변환.
...
- 카테고리 매핑(용어 정규화): 사용자의 구체적인 물품명(예: 고추장, 갤탭)을 LLM을 거쳐 DB에 존재하는 공식 데이터셋의 전문 용어(예: 액체·분무·겔류, 전자기기)로 1차 매핑.
- 하이브리드 검색(Hybrid Search): 임베딩을 통한 의미 기반 벡터 검색(Dense)과, 키워드 일치율을 보는 전통적 검색(Sparse, 예: BM25)을 결합하여 동의어 검색 누락을 최소화.
